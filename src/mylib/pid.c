...
bool PID_Compute(PidType* pid) {
  ...
  if (!pid->inAuto || !pid->Enable) {
    return FALSE;
  }

  	if(TIMER_Over(&pid->SampleTimer))
  	{
		TIMER_Start(&pid->SampleTimer, pid->SampleTime);
...
{
    uint8_t newAuto;
    newAuto = (Mode == PID_Mode_Automatic);
...
void PID_Reset_Values(PidType* pid)
{
	  pid->myInput = 0;
	  pid->myOutput = 0;
	  pid->mySetpoint = 0;
